...
{
    int fd;
    char buf[64*1024] __attribute__((aligned(32)));
    char resout[32*1024 + 64] __attribute__((aligned(32)));
    size_t pos;
    ssize_t inpos;
    ssize_t incnt;
    int     count;
...
struct open_file *fid_to_open_file[1024];
...
static void
set_file(FILE *fp, struct open_file *of)
{
   int fd = fileno(fp);
   bzero(&of->hs, sizeof of->hs);
   of->hs.fd = fd;
   fid_to_open_file[fd] = of;
}
...
get_file_from_fp(FILE *fp)
{
   int fd = fileno(fp);
   struct open_file *of = fid_to_open_file[fd];

   if (of == NULL) {
      /* Sed doesn't seem to register these, so do them
       * separately
       */
      if (fp == stdin || fp == stdout) {
         of = MALLOC(1, struct open_file);
         of->link = NULL;
         of->name = fp == stdin ? strdup("stdin") : strdup("stdout");
         of->fp = fp;
         of->temp = false;
         set_file(fp, of);
      }
      else {
         register_open_file (fp, utils_fp_name(fp), false);
      }
      of = fid_to_open_file[fd];
   }
   return of;
}
...
static void
of_flush(struct hacked_store *hs)
{
   if (write(hs->fd, hs->buf, hs->pos) < 0)
     panic(_("couldn't write file: %s"), strerror(errno));
   hs->pos = 0;
}
...
ck_fwrite(ptr, size, nmemb, stream)
  ...
  FILE *stream;
{
   struct open_file *of = get_file_from_fp(stream);
   struct hacked_store *hs= &of->hs;

   memcpy(hs->buf + hs->pos, ptr, size*nmemb);
   hs->pos += size*nmemb;
   hs->buf[hs->pos] = '\0';
   if (hs->pos > 32*1024)
      of_flush(hs);
  ...
  FILE *stream;
{
  ...
  if (size && (nmemb=fread_unlocked(ptr, size, nmemb, stream)) <= 0 && ferror_unlocked(stream))
...
static int
read_proc_buf(struct hacked_store *hs)
{
   char resbuf[32] __attribute__((aligned(32)));
  __m128i nl = _mm_set1_epi8('\n');
  __m128i *a = (__m128i *)hs->buf;
  __m128i *res = (__m128i *)resbuf;
  ssize_t nread;
  int count;
  char *r = hs->resout;

  if ((nread = read(hs->fd, hs->buf, 32*1024)) <= 0) {
     return -1;
  }

  if (nread < 32*1024)
     memset(hs->buf + nread, 0, 32*1024-nread);

  count = 32*1024/16+1;
  while (--count) {
     *res = _mm_cmpistrm(*a++, nl, _SIDD_CMP_EQUAL_EACH);
     memcpy(r, res, 2);
     r+=2;
  }

  hs->count = 32*1024/8/8;
  hs->p = 0;
  hs->n = 0;
  return 0;
}
  ...
  FILE *stream;
{
  ...
  *buflen = 0;
  ...
  for (;;) {
     ...
     if (hs->wrapped_size <= hs->texti + 32*1024) {
        hs->wrapped_size = (hs->texti + 32*1024 - 1)/(32*1024) * (32*1024) + 32*1024;
        hs->wrapped_text = realloc(hs->wrapped_text, hs->wrapped_size);
     }

     r = (uint64_t *)hs->resout + (32*1024/8/8 - hs->count);
     while (hs->count) {
        if (*r != 0) {
           ind = __builtin_ctzl(*r);
           *r &=~ 1ul<<ind;
           strlen = hs->p + ind + 1 - hs->n;

           /* Handle wrapped text */
           if (hs->texti != 0) {
              memcpy(hs->wrapped_text + hs->texti, hs->buf, strlen);
              *text = hs->wrapped_text;
              hs->n = strlen;
              strlen += hs->texti;
              hs->texti=0;
              return strlen;
     ...
     /* Handle wrapped text */
     memcpy(hs->wrapped_text + hs->texti, hs->buf + hs->n, hs->p - hs->n);
     hs->texti += hs->p - hs->n;

     if (read_proc_buf(hs) < 0)
        return -1;
  }
}
  ...
  FILE *stream;
{
  ...
  clearerr_unlocked(stream);
  if (fflush_unlocked(stream) == EOF && errno != EBADF)
  ...
  while ( (cur = prev->link) )
      ...
      if (!stream || stream == cur->fp)
	{
	  ...
	  prev->link = cur->link;
          ...
          if (cur->hs.wrapped_text != NULL) {
             free(cur->hs.wrapped_text);
             cur->hs.wrapped_size = 0;
          }

          fid_to_open_file[cur->hs.fd] = NULL;
	  free(cur->name);
	  free(cur);
	}
      else
  ...
  if (!stream)
    {
      ...
      cur = fid_to_open_file[0];
      if (cur != NULL) {
         if (cur->hs.wrapped_text != NULL) {
            free(cur->hs.wrapped_text);
            cur->hs.wrapped_size = 0;
         }
         fid_to_open_file[0] = NULL;
         free(cur->name);
         free(cur);
      }
      cur = fid_to_open_file[1];
      if (cur != NULL) {
         if (cur->hs.wrapped_text != NULL) {
            free(cur->hs.wrapped_text);
            cur->hs.wrapped_size = 0;
         }
         fid_to_open_file[1] = NULL;
         free(cur->name);
         free(cur);
      }
      cur = fid_to_open_file[2];
      if (cur != NULL) {
         if (cur->hs.wrapped_text != NULL) {
            free(cur->hs.wrapped_text);
            cur->hs.wrapped_size = 0;
         }
         fid_to_open_file[2] = NULL;
         free(cur->name);
         free(cur);
      }
    }
...
do_ck_fclose(fp)
  FILE *fp;
{
  struct open_file *of = get_file_from_fp(fp);
  struct hacked_store *hs= &of->hs;
  ck_fflush(fp);
